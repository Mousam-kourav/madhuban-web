...
function getPresignLimiter(): Ratelimit | null {
  ...
  _limiter = new Ratelimit({
    ...
    prefix: 'madhuban:featured-presign',
  });
  return _limiter;
}
...
export async function POST(request: NextRequest) {
  ...
  const slug = slugify(slugInput || filename);
  if (!slug) return NextResponse.json({ error: 'slug or filename is required' }, { status: 400 });

  const ext = EXT_BY_MIME[mimeType] ?? 'bin';
  const r2Key = `featured-experiences/${slug}/originals/${slug}-${uid8()}.${ext}`;
